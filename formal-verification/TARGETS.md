...
1. **Task 5** (Proofs for `committed_index`) — sort-then-index majority commit algorithm; the Aeneas-safe variant in `majority.rs` is a clean Vec-based implementation.
2. **Task 5** (Proofs for `joint_committed_index`) — builds on `committed_index`; just takes `min` of two committed indices.
