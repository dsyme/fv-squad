...
548~theorems across 48~Lean~4 files (stdlib only, no Mathlib),
...
reachability model.  An additional 13 correspondence-validation files
provide 204 compile-time \texttt{\#guard} assertions and 13 Rust test
functions, confirming that the Lean models agree with the Rust
implementation on concrete inputs across all major targets.

The total LLM token cost was approximately \$490 (70~runs at
...
proof engineering.  His total intervention over the project's 70~runs
...
\item The entire effort is \emph{automated}: every target selection,
  ...
  LLM-driven FV agent across 70 independent runs, with only two brief
...
\midrule
1 & Data Structures     & 5  & 135 \\
2 & Quorum Arithmetic   & 7  & 124 \\
3 & Protocol Operations & 4  & 58  \\
4 & Cross-Module Composition & 3 & 26 \\
5 & Raft Safety         & 2  & 18  \\
6 & Reachability        & 1  & 4   \\
7 & Election Model (complete) & 10 & 114 \\
8 & Correspondence Validation & 13 & 2+204\texttt{g} \\
9 & ReadOnly Operations & 1  & 13  \\
10& FindConflictByTerm  & 1  & 10  \\
\midrule
  & \textbf{Total}      & \textbf{48} & \textbf{548+204\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 204 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they contribute 2 named theorems (in \texttt{FindConflictCorrespondence.lean}).}
\label{tab:layers}
\end{table}
...
independent confirmation.  Together, the 204 Lean \texttt{\#guard}
...
Thirteen correspondence-test files provide compile-time evidence that the
...
The 13 files cover: \texttt{find\_conflict} (20~guards, exact),
...
\texttt{read\_only} (15~guards, exact), and
\texttt{find\_conflict\_by\_term} (16~guards, abstraction).
In total: 204 compile-time correspondence assertions and 13 Rust test functions.

\paragraph{Layer~9: ReadOnly Operations.}
\texttt{ReadOnly.lean} (13~theorems) formalises \texttt{raft-rs}'s
...
Total: 548 theorems, 48~files (35 proof files + 13 correspondence-test files),
0 \texttt{sorry}, $\approx$14{,}000 lines of Lean.
...
ReadOnly          & 13 & RO8: advance removes ctx; RO14--RO15: inv \\
FindConflictByTerm & 10 & FCB3: maximality; FCB6: always some \\
FindConflictCorrespondence &  2 & 27 \texttt{\#guard}, exact \\
...
\midrule
\textbf{Total}    & \textbf{548} & \textbf{0 sorry; 204 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
...
All 548~theorems are consistent with the Rust implementation, as further
confirmed by 204 compile-time \texttt{\#guard} correspondence assertions
...
specifications, all Lean formalisations, all 548 proofs, the 13
...
Across 70 runs spanning $\approx$11 weeks, the agent produced:
\begin{itemize}\setlength\itemsep{0pt}
  \item 26 informal specification documents
  \item 48 Lean~4 source files ($\approx$14{,}000 lines)
  \item 548 machine-checked theorems
  \item 204 compile-time \texttt{\#guard} correspondence assertions
...
Finding no implementation bugs in 548~theorem-checks is itself evidence
...
\emph{diverges}: the Lean Squad agent completed all 548~theorems
...
Squad accumulates knowledge across 70 independent GitHub Actions runs
...
Our $\approx$14{,}000 lines of Lean and 548 theorems are modest by comparison.
...
70 LLM agent runs (each $\sim$15--30 minutes of wall-clock time)
...
and produced 548 machine-checked theorems across 48 files in 70 runs,
...
reachability model, and an additional 13 correspondence-validation
files providing 204 compile-time \texttt{\#guard} assertions and 13
...
specifications, all 548 proofs, the ten-layer architecture, and the
