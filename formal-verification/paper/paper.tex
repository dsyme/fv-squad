...
merging pull requests, and writing two short comments across 115
autonomous agent runs.

The agent autonomously researched the codebase, selected 26 verification
targets, designed a twenty-five-layer proof architecture (data structures, quorum arithmetic,
...
940~theorems across 79~Lean~4 files (stdlib only, no Mathlib),
...
provide 642 compile-time \texttt{\#guard} assertions and 18+ Rust test
functions, confirming that the Lean models agree with the Rust
implementation on concrete inputs across all major targets.

The total LLM token cost was approximately \$805 (115~runs at
...
the twenty-five-layer proof architecture, specification writing, or any actual
...
\item The entire effort is \emph{automated}: every target selection,
  ...
  LLM-driven FV agent across 115 independent runs, with only two brief
...
runs: the twenty-five-layer architecture, the choice of targets, and the
...
8 & Correspondence Validation & 28 & 4+642\texttt{g} \\
...
21& Election Lifecycle (A7)   & 1  & 10 \\
22& Progress Set             & 1  & 10 \\
23& MemStorage Core          & 1  & 16 \\
24& IsContinuousEnts         & 1  &  8 \\
25& Restore                  & 1  &  8 \\
\midrule
  & \textbf{Total}      & \textbf{79} & \textbf{940+642\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 642 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they contribute 4 named theorems (in \texttt{FindConflictCorrespondence.lean}).}
\label{tab:layers}
\end{table}
...
independent confirmation.  Together, the 642 Lean \texttt{\#guard}
...
\texttt{RaftLogAppend.lean} (19~theorems) formalises
...
In total: 642 compile-time correspondence assertions and 18+ Rust test functions.
...
\texttt{RaftLog::truncate\_and\_append}
...
characterise the \texttt{hasNextEntries\-Since} family; HNE14 gives the
top-level conjunction form used by NE1.

\paragraph{Layer~21: Election Lifecycle (A7 Bridge).}
\texttt{ElectionLifecycle.lean} (10~theorems, EL1--EL7) is the capstone file
that closes the A7 gap identified by external critique.  It defines
\texttt{ElectionEpoch}---a structure tying a concrete Raft election
(\texttt{wonInTerm}) to a \texttt{BroadcastSeq} satisfying ABI8/EBC6
preconditions.  The headline theorem \texttt{fullProtocolStep\_safe} (EL7)
proves that any \texttt{ValidAEStep} from a post-broadcast state is
\texttt{RaftReachable} with \emph{no abstract axioms remaining}.  This makes
the entire Raft safety proof chain fully self-contained.

\paragraph{Layer~22: Progress Set.}
\texttt{ProgressSet.lean} (10~theorems, PS1--PS8) formalises the
...
Correspondence validated by \texttt{ProgressSetCorrespondence.lean} (30
\texttt{\#guard}).

\paragraph{Layer~23: MemStorage Core.}
\texttt{MemStorage.lean} (16~theorems) models \texttt{MemStorageCore}
...
validated by \texttt{MemStorageCorrespondence.lean} (25~\texttt{\#guard}).

\paragraph{Layer~24: IsContinuousEnts.}
...
Total: 940 theorems, 79~files (51 proof files + 28 correspondence-test files),
0 \texttt{sorry}, $\approx$21{,}300 lines of Lean.
...
ElectionLifecycle &  10 & EL7: fullProtocolStep\_safe (A7 closed) \\
ProgressSet       &  10 & PS1--PS8: composed progress set \\
MemStorage        &  16 & Compact/append contiguity \\
IsContinuousEnts  &   8 & ICE1--ICE8: batch concat validity \\
Restore           &   8 & RST1--RST8: snapshot restore spec \\
ProgressSetCorrespondence & 0 & 30 \texttt{\#guard}, abstraction \\
MemStorageCorrespondence  & 0 & 25 \texttt{\#guard}, abstraction \\
\midrule
\textbf{Total}    & \textbf{940} & \textbf{0 sorry; 642 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
...
All 940~theorems are consistent with the Rust implementation, as further
confirmed by 642 compile-time \texttt{\#guard} correspondence assertions
...
The twenty-five-layer architecture ensures that every theorem in upper layers has
...
587 \texttt{\#guard} assertions), but do not use mechanical extraction
...
repository, and reviewed merged PRs.  Over the entire 115-run project,
...
extended the election model layer to 175 new theorems including
the A5 bridge (CPS2), A6 term safety (MC4), and the A7 election
lifecycle bridge (EL1--EL7) that closed all five
\texttt{RaftReachable.step} hypotheses.
...
26 FV targets, the twenty-five-layer proof architecture, all 26 informal
specifications, all Lean formalisations, all 940 proofs, the 26
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item 940 machine-checked theorems
  \item 642 compile-time \texttt{\#guard} correspondence assertions
...
infrastructure.  The twenty-five-layer architecture emerged organically from
...
Finding no implementation bugs in 940~theorem-checks is itself evidence
...
Each of the 115 Lean Squad runs consumed approximately \$7 of LLM
token cost at Claude Sonnet~4.6 pricing (at time of publication), for a total of
approximately~\$805.  This covers the full pipeline: reading
...
decisions---stdlib-only (no Mathlib), the twenty-five-layer decomposition, the
...
\emph{diverges}: the Lean Squad agent completed all 940~theorems
...
Squad accumulates knowledge across 115 independent GitHub Actions runs
...
Our $\approx$21{,}000 lines of Lean and 680 theorems are modest by comparison.
...
115 LLM agent runs (each $\sim$15--30 minutes of wall-clock time)
...
a twenty-five-layer proof architecture, wrote all specifications and proofs,
and produced 940 machine-checked theorems across 79 files in 129 runs,
...
files providing 642 compile-time \texttt{\#guard} assertions and 18+
...
specifications, all 940 proofs, the twenty-five-layer architecture, and the
