...
provide 216 compile-time \texttt{\#guard} assertions and 13 Rust test
functions, confirming that the Lean models agree with the Rust
implementation on concrete inputs across all major targets.

The total LLM token cost was approximately \$483 (69~runs at
...
proof engineering.  His total intervention over the project's 69~runs
...
\item The entire effort is \emph{automated}: every target selection,
  ...
  LLM-driven FV agent across 69 independent runs, with only two brief
...
8 & Correspondence Validation & 13 & 2+192\texttt{g} \\
9 & ReadOnly Operations & 1  & 13  \\
10& FindConflictByTerm  & 1  & 10  \\
\midrule
  & \textbf{Total}      & \textbf{48} & \textbf{548+192\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 192 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they contribute 2 named theorems (in \texttt{FindConflictCorrespondence.lean}).}
\label{tab:layers}
\end{table}
...
independent confirmation.  Together, the 192 Lean \texttt{\#guard}
...
In total: 192 compile-time correspondence assertions and 13 Rust test functions.
...
\midrule
\textbf{Total}    & \textbf{548} & \textbf{0 sorry; 192 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
...
confirmed by 192 compile-time \texttt{\#guard} correspondence assertions
...
Squad accumulates knowledge across 69 independent GitHub Actions runs
...
69 LLM agent runs (each $\sim$15--30 minutes of wall-clock time)
...
and produced 548 machine-checked theorems across 48 files in 69 runs,
...
files providing 192 compile-time \texttt{\#guard} assertions and 13
