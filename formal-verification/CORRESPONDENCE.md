...
| `Inflights.lean` *(open PR)* | `src/tracker/inflights.rs` `Inflights` | Abstraction | 15 | Ring-buffer layout abstracted; `set_cap` omitted; sortedness not enforced by type |

---

## `formal-verification/lean/FVSquad/Inflights.lean` *(phase 3 -- open PR)*
...
| Lean name | Rust name | Rust location | Correspondence | Notes |
|-----------|-----------|---------------|----------------|-------|
| `Inflights` (structure) | `Inflights` | `src/tracker/inflights.rs#L1` | Abstraction | Ring buffer abstracted as `{ queue : List Nat, cap : Nat }` |
| `add` | `Inflights::add` | `src/tracker/inflights.rs#L64` | Abstraction | Appends to queue tail; ring-buffer index arithmetic omitted |
| `freeTo` / `dropLeq` | `Inflights::free_to` | `src/tracker/inflights.rs#L77` | Abstraction | Drops prefix <= `to`; `maybe_free_buffer` omitted |
| `freeFirstOne` | `Inflights::free_first_one` | `src/tracker/inflights.rs#L101` | Abstraction | `freeTo` applied to head element |
| `reset` | `Inflights::reset` | `src/tracker/inflights.rs#L110` | Exact | Clears queue; `cap` preserved |

#### Known divergences (Abstraction-level)

1. **Ring-buffer layout**: Rust uses `start`, `count`, `buffer` circular-Vec. Lean uses `List Nat`. Ring-buffer index arithmetic abstracted away.
2. **`incoming_cap` / `set_cap`**: Dynamic capacity changes not modelled.
3. **`u64` -> `Nat`**: No overflow modelled.
4. **Panics omitted**: `add` on full panics in Rust; Lean precondition `count < cap` rules this out.
5. **Sortedness not enforced by type**: INF8/INF9 take sortedness as a hypothesis.

#### Impact on proofs

All 15 theorems (INF1-INF15) are sound under the abstract model. INF4 (count <= cap) and INF8 (freeTo all-gt-sorted) are the most protocol-relevant. A phase-4 proof would establish observational equivalence between the concrete ring-buffer and the abstract List model.
